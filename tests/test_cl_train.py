...
def test_lbert_cl_train():
    train_main(TrainArguments(CustomModelType.lbert_no_head, 
                              dataset=[CustomDatasetName.tenk_pretrained_mini],
                              train_dataset_sample=100, eval_steps=5))
    torch.cuda.empty_cache()
    train_main(TrainArguments(CustomModelType.lbert_no_head, dataset=[CustomDatasetName.tenk_pretrained_mini],
